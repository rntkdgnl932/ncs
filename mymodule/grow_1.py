...
def tuto_grow(cla):
    try:
        ...
        if result_ == False:
            ...
            # 전직
            full_path = "c:\\nightcrow\\imgs\\grow\\grow_1\\class_change_1.PNG"
            img_array = np.fromfile(full_path, np.uint8)
            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
            ...
            if imgs_ is not None and imgs_ != False:
                ...
                time.sleep(0.7)
                full_path = "c:\\nightcrow\\imgs\\grow\\grow_1\\class_change_2.PNG"
                img_array = np.fromfile(full_path, np.uint8)
                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
...
def quest_check(cla):
    try:
        ...
        # 퀘스트 수락
        full_path = "c:\\nightcrow\\imgs\\grow\\grow_1\\quest_soolock_1.PNG"
        img_array = np.fromfile(full_path, np.uint8)
        img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
...
def knight_of_death_alrim(cla):
    try:
        ...
        full_path = "c:\\nightcrow\\imgs\\grow\\grow_1\\knightofdeath_2.PNG"
        img_array = np.fromfile(full_path, np.uint8)
        img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
        ...
        full_path = "c:\\nightcrow\\imgs\\grow\\grow_1\\knightofdeath_1.PNG"
        img_array = np.fromfile(full_path, np.uint8)
        img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
