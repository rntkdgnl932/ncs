...
def line_monitor(game, cla):
    ...
    try:
        ...
        while isLoop is False:
            ...
            print(nowTime)
            full_path = "c:\\nightcrow\\imgs\\monitor\\unreal_error_1.png"
            ...
            full_path = "c:\\nightcrow\\imgs\\monitor\\closewithoutsending.PNG"
            img_array = np.fromfile(full_path, np.uint8)
            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
            imgs_ = imgs_set(0, 700, 960, 850, "one", img)
            if imgs_ is not None:
            ...
            full_path = "c:\\nightcrow\\imgs\\monitor\\unreal_error_2.png"
            img_array = np.fromfile(full_path, np.uint8)
            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
            ...
            full_path = "c:\\nightcrow\\imgs\\monitor\\sendandrestart.PNG"
            img_array = np.fromfile(full_path, np.uint8)
            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
            imgs_ = imgs_set(0, 700, 960, 850, "two", img)
            if imgs_ is not None:
            ...
            # 장시간
            jangsigan = False
            full_path = "c:\\nightcrow\\imgs\\monitor\\jangsigan_1.PNG"
            img_array = np.fromfile(full_path, np.uint8)
            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
